...
func Session(url string, capabilities interface{}, v interface{}) error {
	json_bytes, err := json.Marshal(capabilities)
	if err != nil {
	...
	response, err := http.Post(url+"/session", "application/json", buffered_data)
	if err != nil {
...
func free_port() (int, error) {
	...
	tcp_listener, err := net.ListenTCP("tcp4", &tcp_address)
	if err != nil {
		return 0, errors.New("Error: unable to listen on ephemeral port")
	}
	defer tcp_listener.Close()
	port, err := strconv.Atoi(tcp_listener.Addr().String()[strings.LastIndex(tcp_listener.Addr().String(), ":")+1:])
	if err != nil {
...
func getChromeDriver(latest bool) (driver_path string, err error) {
	...
	chrome_driver_url, driver_version, err := linuxChrome64DriverURL(latest)
	if err != nil {
	...
	if _, err := os.Stat(driver_dir); err != nil && os.IsNotExist(err) {
		file_info, err := os.Stat(home_dir)
	...
	if _, err := os.Stat(driver_path); err != nil && os.IsNotExist(err) {
		...
		zip_file, err := ioutil.TempFile("", zip_name)
		if err != nil {
		...
		resp, err := http.Get(chrome_driver_url)
		if err != nil {
		...
		zip_reader, err := zip.OpenReader(zip_path)
		if err != nil {
		...
		for _, file := range zip_reader.File {
			...
			chrome_driver, err := os.Create(file_path)
			if err != nil {
			...
			chrome_driver.Close()
			file_info, err := os.Stat(driver_path)
			if err != nil {
...
func startChromeDriver(latest bool, port int) error {
	...
	driver_path, err := getChromeDriver(latest)
	if err != nil {
